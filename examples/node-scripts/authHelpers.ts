...
export const getCredentialsFromEnvOrThrow = () => {
  ...
  if (!apiTokenClientId || !apiTokenClientSecret || !walletNodeId) {
    throw new Error(
      "Missing test credentials. Please set LIGHTSPARK_API_TOKEN_CLIENT_ID, LIGHTSPARK_API_TOKEN_CLIENT_SECRET, and LIGHTSPARK_WALLET_NODE_ID environment variables."
    );
  }
  return {
    apiTokenClientId,
