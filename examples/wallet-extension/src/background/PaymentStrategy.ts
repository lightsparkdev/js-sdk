...
export interface PaymentStrategy {
    onPlayedRange(previousRanges: PlaybackRange[], newRanges: PlaybackRange[]): OmitTypename<CurrencyAmount>;
}
...
export class LinearPaymentStrategy implements PaymentStrategy {
    ...
    constructor(paymentPerChunk: OmitTypename<CurrencyAmount>, chunkSizeSec: number) {
    ...
    private numberOfChunksInRanges(ranges: PlaybackRange[]): number {
        const totalDuration = ranges.reduce((acc, range) => acc + range.duration(), 0);
