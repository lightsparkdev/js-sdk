 ...
 * @typedef {Object} Seed
 * @property {string} the seed raw bytes.
 */
export type Seed = {
    seed: Uint8Array;
}
 ...
 */
export async function generateMnemonic(): Promise<Mnemonic> {
    return await LightsparkdevReactNativeCryptoModule.generateMnemonic();
}
 ...
 */
export async function derivePublicKey(seed: Seed, derivationPath: string): Promise<string> {
    return await LightsparkdevReactNativeCryptoModule.derivePublicKey(seed, derivationPath);
}
 ...
 * @param multiTweak An optional tweak to multiply the private key by.
...
export async function deriveKeyAndSign(
    ...
    addTweak?: Uint8Array,
    multiTweak?: Uint8Array
  ): Promise<Uint8Array> {
    return await LightsparkdevReactNativeCryptoModule.deriveKeyAndSign(
        seed, message, derivationPath, addTweak, multiTweak
    );
}
...
export async function ecdh(
    ...
    return await LightsparkdevReactNativeCryptoModule.ecdh(seed, derivationPath, publicKey);
}