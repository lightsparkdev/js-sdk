...
/**
 * Generate a key pair for signing and ensures that the private key is stored
 * in the native keystore. After running this function,
 * the alias in the {@link ExportedKeys.alias} property of the returned object
 * can be used to signing messages.
 *
 * You likely want to store the alias in a a persistent local store (like
 * AsyncStorage) so that you can use it later.
 *
...
/**
 * Exports the raw byes of the key pair with the given alias.
 * This should only be used for debugging purposes or for backing up the key
 * pair. Do not keep these bytes in memory for longer than necessary.
 *
 ...
 *
 * You likely want to store the returned alias in a a persistent local store
 * (like AsyncStorage) so that you can use it later.
 *
 * NOTE: iOS can only import pkcs1 format private keys! Other formats will
 * fail. However, if you're just importing a key that was generated by this
 * native crypto module, it will be in the correct format.
 *
...
/**
 * Derives the private key from a seed and a derivation path,
 * then sign a message with it.
 *
 * @param seed The master seed.
